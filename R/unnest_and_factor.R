...
unnest_and_factor <- function(df,var,sep) { #where df is the dataset, var is the variable to be unnested and sep is the separator used in the observations of the nested variable
  ...
  unnested_factored <- df %>%
    dplyr::mutate({{var}}:=strsplit(as.character({{var}}), sep))%>%  #separating the observations on the given separator
    tidyr::unnest({{var}}) %>% #unnesting the separated observations
    dplyr::mutate({{var}}:=as.factor({{var}})) #factoring the observations for grouping capabilities
